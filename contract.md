...
- `content/work.ts` holds the five portfolio summaries, internal case-study URLs, project artwork and UpForIt image assets, with Good Game Apparel first and categorised as “Print on Demand Merch Service”. The summaries describe each project's sector and Brandd's contribution. SonaCrate remains in the wider portfolio with its existing music-platform description and is not a homepage highlight.
...
- A compact light hero presents “Websites and software built around your business.” Its primary Discuss a project link goes to Contact; See our work goes to `/projects`.
- The hero uses `public/images/brandd/homepage-banner.png`, generated artwork showing an illustrative business website, online shop and connected order-management screen in Brandd colours. On desktop the devices sit on the right, with white space behind the text. The laptop faces forward while the phone and tablet have opposing rotations and perspective for depth. These are service illustrations rather than client screenshots. The decorative image uses Next.js image optimisation, responsive image sizes and an empty alt attribute. Below 1081px the artwork sits beneath the full-width text and enquiry actions; its blank left canvas is cropped so all three devices fit within the screen.
- The “See what we’ve built.” section immediately follows the hero without an eyebrow label. Three cards feature Good Game Apparel, UpForIt and Ace Hits TCG in that order, with artwork, project type, Brandd's role and internal case-study links covering the full card. These are live businesses/services. SonaCrate appears only in the wider portfolio. An All our work link leads to the full portfolio.
...
- The homepage has a white hero with a gradient border and Brandd-colour artwork beside the copy. The visible device group occupies a responsive desktop column up to 800px wide, anchored closer to the text on large screens. The image is sized around its right-side artwork rather than its blank canvas, preserving the device angles and a clear gap from the copy. A subtle cyan, violet and pink glow sits behind the devices and fades to white on the left; multiply blending lets it show through the image's white canvas. At widths up to 1080px, the hero stacks the copy and actions above a centred artwork panel up to 600px wide with a 10:7 aspect ratio. The image shifts left to show the full device group, and the glow fades at the panel edges. Standalone `ScrollBridge` transitions and decorative rails are not rendered on the current pages.
- `light-section` and `dark-section` establish section tone; major sections expose `data-nav-tone` for header contrast.
- Portfolio cards have a three-column desktop grid, two columns at tablet widths and one on phones. Each card is a single named case-study link covering the artwork, copy and padding, with a cyan border on hover and an outer outline for keyboard focus. The View case study text remains a visual cue within the same link.
