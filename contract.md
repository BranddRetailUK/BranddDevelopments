...
- `next.config.ts` disables the `X-Powered-By` header, redirects `www.brandd.co.uk` to `brandd.co.uk`, and sends security headers on all routes. `middleware.ts` enforces a nonce-based Content Security Policy, redirects non-canonical page hosts, rejects API calls sent to non-approved hosts, and lets Railway call the dedicated `/api/health` liveness route. `railway.json` runs migrations before deployment and configures that health check.
...
- The footer repeats the no-strap white logo, top-level navigation links, a Privacy link, and a permanent Cookie settings control that reopens consent preferences.
...
- The page starts with a dark contact section that places `ContactForm` before the direct email and project enquiry cards.
...
- Budget range options are selected through checkbox-style required controls and include £0-£1,000, £1,000-£5,000, £10,000-£20,000, and £20,000+.
- The supporting cards include a visible direct email option linking to `enquiries@brandd.co.uk`.
...
- Submitting the form posts to `POST /api/contact`, disables the submit button while sending, resets the form after a successful save, changes the submit button into a persistent green success state with a sweeping flare and centered check icon until page reload, fires consent-gated `generate_lead` data-layer/gtag measurement, fires a consent-gated Google Ads `conversion` event with `send_to` built from the public Ads ID and conversion-label environment variables when configured, and shows success or error feedback.
- The form submits current-page attribution with the enquiry, including landing page, page path, referrer, UTM fields, Google click identifiers (`gclid`, `gbraid`, `wbraid`), and consent choice. First-touch attribution is persisted in browser storage only after optional measurement consent.
- The form creates a stable idempotency key for each submission attempt, includes a hidden anti-bot field and timing signal, and renders Cloudflare Turnstile when its public site key is configured.
- The API rejects unsupported content types and oversized bodies, validates the payload against shared focus and budget allowlists, applies a Postgres-backed rate limit using the trusted client-IP header, verifies Turnstile when configured, stores each unique enquiry with the selected budget range, lead attribution, status, lead qualification/upload fields, notification fields, IP address, timestamps, and request user agent, and returns the submission id for client-side lead/conversion events. Duplicate retries return the original submission instead of creating or notifying twice.
- SendGrid contact email notifications are controlled by `SENDGRID_API_KEY`, `CONTACT_EMAIL_TO`, and `CONTACT_EMAIL_FROM`. The default sender display name is Brandd Solutions, emails are sent from the configured verified Brandd enquiries sender to the enquiries mailbox, use `Brandd Enquiry <submission id>` as the subject and visible email title, include the selected budget range, include the square Brandd profile icon in the HTML notification header, store the accepted SendGrid message id when available, and set the customer's submitted email as the reply-to address so replies from the enquiries mailbox go back to the sender. Form submissions still succeed when SendGrid is not configured or email delivery fails; failures are logged and recorded on the submission.
...
- SendGrid and WhatsApp notification requests use bounded timeouts. Delivery jobs are claimed safely across replicas, record sent, skipped, retry, and terminal failure states, and do not hold the form response open.
...
- A light privacy content section identifies Brandd Solutions as controller and explains contact enquiries, lawful bases, cookies and analytics consent, security checks, processors and recipients, international-transfer safeguards, exact default retention periods, data-subject rights, consent withdrawal, the absence of significant automated decisions, and the right to complain to the ICO.
- The page ends with a dark CTA linking to Contact.
